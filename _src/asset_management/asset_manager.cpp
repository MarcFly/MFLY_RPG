...
static std::vector<SQUE_CtrlAsset> assets;
static std::vector<SQUE_Dir> directories;
static double unused_time_unload_ms = 5000;
static std::vector<ImportFileFun*> import_funs = { FILE_IMPORTER_TABLE(EXPAND_AS_VALUE) };
static std::vector<LoadUnloadFun*> load_funs = { FILE_LOAD_UNLOAD_TABLE(EXPAND_AS_ENUM) };
static std::vector<LoadUnloadFun*> unload_funs = { FILE_LOAD_UNLOAD_TABLE(EXPAND_AS_VALUE) };
...
const std::vector<SQUE_Dir*> AssetManager_GetBaseDirs()
{
	std::vector<SQUE_Dir*> ret;
...
std::vector<SQUE_CtrlAsset*> AssetManger_GetAssetsDir(const uint32_t dir_id)
{
	std::vector<SQUE_CtrlAsset*> ret;
	for (uint32_t i = 0; i < assets.size(); ++i)
	{
...
static void SetChildDirs(const SQUE_Dir* base_dir, std::vector<const SQUE_Dir*>* dirs)
{
	for (uint32_t i = 0; i < base_dir->children_ids.size(); ++i)
	{
		dirs->push_back(AssetManager_GetDir(base_dir->children_ids[i]));
		SetChildDirs((*dirs)[dirs->size()-1], dirs);
	}
}

void AssetManager_RefreshDirRecursive(const uint32_t dir_id)
{
	std::vector<SQUE_CtrlAsset*> checked;
	std::vector<const SQUE_Dir*> dirs;
	...
	std::vector<SQUE_FW_NewAsset*> ret = SQUE_FS_CheckDirectoryChanges(dirs[0]->location, checked, NULL);
...
{
    SQUE_FS_Init(GetWorkingDirectory());
	const char* exec_path = GetWorkingDirectory();
...
void AssetManager_Update()
{
	...
	for (uint32_t i = 0; i < assets.size(); ++i)
	{
		...
		if (CHK_FLAG(assets[i].status_flags, SQ_AS_DELETED) && !CHK_FLAG(assets[i].status_flags,SQ_AS_LOADED))
		{
			...
			SQUE_Swap(&assets[i], &assets[assets.size()-1]);
			assets.pop_back();
		}
