...
static SQUE_AssetWindow assets_ui;

// Copy basically from asset_manager and add the string macro for icon
#define ASSET_ICON_TABLE(ENTRY) \
        ENTRY(AIC_UNKNOWN, ICON_MD_TEXT_FIELDS) \
		ENTRY(AIC_FOLDER, ICON_MD_FOLDER_OPEN) \
        ENTRY(AIC_META, ICON_MD_TEXT_FIELDS) \
        ENTRY(AIC_CUSTOM, ICON_MD_FILE_UPLOAD) \
        ENTRY(AIC_IMAGE, ICON_MD_IMAGE) \
        ENTRY(AIC_OBJECT, ICON_MD_TEXT_FIELDS) \
        ENTRY(AIC_MATERIAL, ICON_MD_FILE_UPLOAD) \
        ENTRY(AIC_VERT_SHADER, ICON_MD_CODE) \
        ENTRY(AIC_FRAG_SHADER, ICON_MD_CODE) //

static std::vector<const char*> asset_icons = { ASSET_ICON_TABLE(EXPAND_AS_VALUE) };
...
void SQUE_AssetWindow::MenuBar(float dt)
{
	...
	if (ImGui::Button(ICON_MD_REFRESH, button_size)) AssetManager_RefreshDirRecursive(current_folder); // Look for changes in disk
	ImGui::SameLine();
	...
	ImGui::PushItemWidth(button_size.x * 3);
    ImGui::InputTextWithHint("##AssetWindowSearchBar", (search[0]!='\0') ? search : ICON_MD_SEARCH, search, sizeof(search));
...
void SQUE_AssetWindow::DrawSelectableItem(const uint32_t id, const uint32_t type, const char* asset_name, const float last_font_scale)
{
	...
	ImGui::BeginChild(temp_id, ImVec2(s_vec.x,  2.f*s_vec.y/3.f), false, ImGuiWindowFlags_NoInputs | ImGuiWindowFlags_NoScrollbar);
	{
		...
		ImGui::SetWindowFontScale(fs/1.5f);
	...
	cursor.y += - margin * 3.f + 2.f * s_vec.y / 3.f;// ImGui::GetCursorPos().y;
	ImGui::SetCursorPos(cursor);
	...
	if (ItemDraggable::CheckStartDrag())
	{
		selected_asset = id;
		FileDraggable* file = new FileDraggable();
		file->file_id = id;
		UI_StartDraggable(file);
	}
}
...
void SQUE_AssetWindow::UpdateItems(const ImVec2 item_win_size)
{
	...
	ImGui::BeginChild("##AssetsInFolder", item_win_size, true);
	{
		...
		std::vector<SQUE_CtrlAsset*> folder_assets = AssetManger_GetAssetsDir(current_folder);
		...
		if (ImGui::IsMouseClicked(MOUSE_BUTTON_LEFT) && ImGui::IsWindowHovered())
			selected_asset = 0;
	}
	ImGui::EndChild();
}
...
void SQUE_AssetWindow::Update(float dt)
{
	...
	if (ImGui::Begin(name, &active))
	{
		...
		ImGui::BeginChild("##AssetsFoldersHierarchy", child_sizes, true);
		{
			...
			const std::vector<SQUE_Dir*> dirs = AssetManager_GetBaseDirs();
